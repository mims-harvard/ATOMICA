...
class BiolipDataset(torch.utils.data.Dataset):
    def __init__(self, data_files):
        super().__init__()
        self.data_files = data_files
        # aas = [  # amino acids
        #         ('G', 'GLY', 1), ('A', 'ALA', 2), ('V', 'VAL', 3), ('L', 'LEU', 4),
        #         ('I', 'ILE', 5), ('F', 'PHE', 6), ('W', 'TRP', 7), ('Y', 'TYR', 8),
        #         ('D', 'ASP', 9), ('H', 'HIS', 10), ('N', 'ASN', 11), ('E', 'GLU', 12),
        #         ('K', 'LYS', 13), ('Q', 'GLN', 14), ('M', 'MET', 15), ('R', 'ARG', 16),
        #         ('S', 'SER', 17), ('T', 'THR', 18), ('C', 'CYS', 19), ('P', 'PRO', 20) # 20 aa
        #         # ('U', 'SEC') # 21 aa for eukaryote
        #     ]

        # bases = [ # bases for RNA/DNA
        #         ('DA', 'DA', 21), ('DG', 'DG', 22), ('DC', 'DC', 23), ('DT', 'DT', 24), # DNA
        #         ('RA', 'RA', 25), ('RG', 'RG', 26), ('RC', 'RC', 27), ('RU', 'RU', 28)      # RNA
        # ]
        bglb_index = VOCAB.symbol_to_idx(VOCAB.GLB)
        unk_index = VOCAB.symbol_to_idx(VOCAB.UNK) - bglb_index # bglb_index is added back later
        self.biolip_to_vocab = {
        0: unk_index,#small molecule residue, gets deleted with masks
        1: 2,#"ALA"
        2: 16,#"ARG"
        3: 11,#"ASN"
        4: 9,#"ASP"
    ...
    def __getitem__(self, idx):
        ...
        '''
        raw_data = torch.load(self.data_files[idx])["pocket_patch"]
        
        # TODO: add global nodes
        # X: add center of segment 0 and center of segment 1
        # B: add global node for segment 0 and segment 1, global node = 3, shift all other nodes by 3
        # A: add global node for segment 0 and segment 1, global node = 2, shift all other nodes by 2
        # atom_positions: add filler for global node
        # block_lengths: add 1 for global nodes
        # segment_ids: add segment id for global nodes

        block_id = raw_data["residue"].x.flatten()
        segment1a = raw_data["atom"].molecule_index == 0
        segment2a = raw_data["atom"].molecule_index != 0
        segment1b = raw_data["residue"].molecule_index == 0 # torch.logical_and(raw_data["residue"].molecule_index == 0, block_id != 0) # delete residue == 0, this is replaced with global node in GET
        segment2b = raw_data["residue"].molecule_index != 0 # torch.logical_and(raw_data["residue"].molecule_index != 0, block_id != 0) # delete residue == 0, this is replaced with global node in GET

        coord = raw_data["atom"].pos
        X = torch.cat([coord[segment1a].mean(dim=0, keepdim=True), coord[segment1a], coord[segment2a].mean(dim=0, keepdim=True), coord[segment2a]], dim=0)
        bglb_index = VOCAB.symbol_to_idx(VOCAB.GLB)
        
        # Remap from biolip amino acid numbering to GET amino acid numbering
        B = block_id.clone()
        B.apply_(self.biolip_to_vocab.get)
        B = B + bglb_index
        B = torch.cat([torch.tensor([bglb_index]), B[segment1b], torch.tensor([bglb_index]), B[segment2b]], dim=0)

        aglb_index = VOCAB.get_atom_global_idx()
        A = torch.cat([torch.tensor([aglb_index]), raw_data["atom"].x.flatten()[segment1a]+aglb_index, torch.tensor([aglb_index]), raw_data["atom"].x.flatten()[segment2a]+aglb_index], dim=0)
        block_lengths = raw_data["atom", "to", "residue"].edge_index[1].bincount()
        block_lengths = torch.cat([torch.tensor([1]), block_lengths[segment1b], torch.tensor([1]), block_lengths[segment2b]], dim=0)
        segment_ids = raw_data["residue"].molecule_index
        segment_ids = torch.cat([torch.tensor([0]), segment_ids[segment1b], torch.tensor([1]), segment_ids[segment2b]], dim=0)

        assert len(B) == len(block_lengths) == len(segment_ids), "lengths of B, block_lengths, segment_ids should be the same"
        assert len(A) == len(X), "lengths of A and X should be the same"
        return {
            "X": X.unsqueeze(1),
            "B": B,
            "A": A,
            "atom_positions": torch.zeros_like(A),
