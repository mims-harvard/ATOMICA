...
def process_dataset(dataset, modality, go_df, pfam_df):
    ...
    for item in tqdm(dataset, total=len(dataset), desc=f"Processing {modality}"):
        ...
        if pfam_df is not None:
            result["pfam"] = get_block_pfam_domain(pdb_id, item['block_to_pdb_indexes'], pfam_df)
        if go_df is not None:
            result["go"] = get_block_go_labels(pdb_id, item['block_to_pdb_indexes'], go_df)
        output.append(result)
    return output

def main(dataset_path, go_df_path, pfam_df_path, out_path, shard=None, num_shards=None):
    if go_df_path is not None:
        go_df = pd.read_csv(go_df_path, sep="\t", skiprows=1)
        go_df = go_df[go_df['EVIDENCE'].isin(["EXP", "IDA", "IMP", "IGI", "IEP", "ISS", "ISA", "ISM", "ISO"])] # filter by evidence codes
    else:
        go_df = None
    if pfam_df_path is not None:
...
def parse():
    ...
    parser.add_argument("--go_df_path", type=str)
    parser.add_argument("--pfam_df_path", type=str)
