...
def process_one(item, pdb_file):
    ...
    for residue_start in bs.get_residue_starts(atom_array):
        ...
        if f"{chain}_{res_id}" in pdb_indexes_to_block:
            sequence_to_block_idx[len(sequence)] = pdb_indexes_to_block[f"{chain}_{res_id}"]
        sequence += d3to1.get(residue_index, '?')
    
    wt_log_probabilities = encode_one_chain(sequence)
    for seq_idx, block_idx in sequence_to_block_idx.items():
        mutant_sequence = sequence[:seq_idx] + '#' + sequence[seq_idx+1:]
        mt_aa_log_probabilities = encode_one_chain(mutant_sequence)[seq_idx]
        wt_aa_log_probabilities = wt_log_probabilities[seq_idx]
        log_probability_diff = mt_aa_log_probabilities - wt_aa_log_probabilities
        esm_scores[block_idx] = log_probability_diff[alphabet.tok_to_idx[sequence[seq_idx]]]
    return esm_scores


def encode_one_chain(sequence, max_len:int = 1022) -> Tuple[torch.Tensor, str]:
    batch = []
    ...
    logits = torch.cat([logits[i, 1:l-1] for i, l in enumerate(batch_lens)], dim=0)
...
if __name__ == '__main__':
    ...
    out_path = "/n/holylfs06/LABS/mzitnik_lab/Lab/afang/ATOMICA/interact_score/PL_ESM2_scores.pkl"
    ...
    dataset_chunks = len(data)//12 + 1
    start = chunk_idx * dataset_chunks
    end = min((chunk_idx + 1) * dataset_chunks, len(data))

    esm_scores = {}
    for item in tqdm(data[start:end], total=end-start, desc="Processing ESM scores"):
        item_id = item['id']
        pdb_file = os.path.join(pdb_dir, f"{item_id}")
        esm_scores[item_id] = process_one(item, pdb_file)
    
    with open(out_path.replace(".pkl", f"_{chunk_idx}.pkl"), 'wb') as f:
        pickle.dump(esm_scores, f)