...
def process_one(pdb_id, label, benchmark_dir, interface_dist_th, fragment):
    ...
    try:
        list_blocks1, list_indexes = pdb_to_list_blocks(prot_fname, return_indexes=True)
    except Exception as e:
        print_log(f'{pdb_id} protein parsing failed: {e}', level='ERROR')
        return None
    try:
        blocks2, new_mol, frag_mol = mol2_to_blocks(sm_fname, fragment=fragment, get_mol=True)
    except Exception as e:
    ...
    # construct pockets
    blocks1, _, indices1, _ = blocks_interface(blocks1, blocks2, interface_dist_th, return_indexes=True)
    ...
    data['list_indexes'] = list_indexes
    data['indices'] = indices1
    data['new_mol'] = new_mol
...
def main(args):
    ...
    for pdb_id in labels:
        ...
        if item is None:
            continue
        processed_pdbbind[0].append({'id': pdb_id, 'label':labels[pdb_id]})
        item['data']['id'] = pdb_id
        item['data']['label'] = labels[pdb_id]
        processed_pdbbind[1].append(item['data'])
    ...
    if not os.path.exists(out_dir):
        os.makedirs(out_dir)               
    data_out_path = os.path.join(out_dir, 'customized_LBA.LBADataset' + args.fragment + '_processed.pkl') 
    
    data_out = (args.interface_dist_th, processed_pdbbind[0], processed_pdbbind[1])
    print_log(f'Obtained {len(data_out)}, saving to {data_out_path}...')
    with open(data_out_path, 'wb') as fout:
        pickle.dump(data_out, fout)
