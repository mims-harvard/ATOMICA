...
def get_disease_connectivity(modality, disease_id, seed=-1):
    ...
    if seed != -1:
        # node permutation = shuffle uniprot ids assigned to nodes
        uniprot_to_node_idx = modality_graphs[modality]['uniprot_to_node_idx']
        keys = list(uniprot_to_node_idx.keys())
        values = list(uniprot_to_node_idx.values())
        random.seed(seed)
        random.shuffle(values)
        uniprot_to_node_idx = dict(zip(keys, values))
...
disease_connectivity_df.to_csv(f"/n/netscratch/mzitnik_lab/Lab/afang/InteractNN/protein_universe/function/disgenet/node_permutation_test/disease_connectivity_pesto_{pesto_cutoff}_plddt_{plddt_cutoff}_fix_irreps_ensemble_{cpu_id}.csv", index=False)